...
class Fuel_posts extends Fuel_base_library {
    ...
    public function get_archives($year = NULL, $month = NULL, $day = NULL, $limit = NULL, $offset = NULL, $order_by = NULL) {
        ...
        if (!empty($year))
            $model->db()->where('YEAR(' . $table_name . '.' . $order_by_field . ') = ' . $year);
        if (!empty($month))
            $model->db()->where('MONTH(' . $table_name . '.' . $order_by_field . ') = ' . $month);
        if (!empty($day))
            $model->db()->where('DAY(' . $table_name . '.' . $order_by_field . ') = ' . $day);
        if (!empty($limit)) {
